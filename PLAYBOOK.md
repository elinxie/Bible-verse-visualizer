...
| — | NEXT SESSION | Re-scanned GitHub issues 2026-08-09: still 0 open (only #7 ever filed, closed, already shipped) — nothing new to slot. Joshua 2/6 and Judges 4/7/16 are all on `main` and live. Continue the "Then" roadmap with **Ruth 1–4** next (the next unchecked item after Judges — short book, 4 chapters, likely fits in one batch). Also worth a periodic check: confirm no further un-reviewed direct merges to main have introduced regressions, and watch for the duplicate-PR / unmerged-PR / draft-PR-left-unmarked-ready pileup pattern (iterations 21, 22, 24, 26, 28, and 29 have each found and cleaned up a stale, unmerged, or still-draft PR from an earlier run — if a PR is open at session start, verify it locally, mark it ready if it's a draft, and merge it before starting new work rather than re-deriving its scope). `node tests/smoke.js` catches data/logic regressions but nothing runs it in CI yet (Tier 3 gate still open; only `deploy.yml`'s post-deploy `verify` job exists, which just polls the live URL, and it only triggers on push to `main`, not on PRs). | ≤1 sonnet | pending |
...
**Then (order):** [x] Exodus 3(M), [x] 12(M), [x] 14(M), [x] 19(M), [x] 20(M), [x] 32(M) · [x] Joshua 2(M), 6(M) · [x] Judges 4(M), 7(M), 16(M) · Ruth 1–4 · 1 Kgs 3, 8, 17–19 · 2 Kgs 5, 18–19, 25 · Ezra 3 · Neh 2, 8 · Esther 4 · Job 1–2, 38 · Pss 22, 23, 51, 137 · Isa 6, 40, 53 · Jer 1, 29, 31 · Ezek 1, 37 · Dan 1–6 · Jonah 1–4 · Mic 5 · Hag 1 ·
...
**Datasets:** places 144/200 (+3 from iteration 28: kishon, harosheth-hagoyim, sorek) · people 206/120 (target exceeded — target was a floor, not a cap; +6 from iteration 28: barak, sisera, jael, delilah, midianites, philistine-lords) · objects 93/60 (target exceeded; +5 from iteration 28: tent-peg, torch-jar, delilah-tests, razor, temple-pillars)
