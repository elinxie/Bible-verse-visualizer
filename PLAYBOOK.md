...
| — | NEXT SESSION | Re-scanned GitHub issues 2026-08-05: still 0 open (only #7 ever filed, closed, already shipped) — nothing new to slot. Exodus 3 and 12 are both on `main` and live. Continue the "Then" roadmap with Exodus 19–20, 32 next. Also worth a periodic check: confirm no further un-reviewed direct merges to main have introduced regressions, and watch for the duplicate-PR / unmerged-PR pileup pattern (iterations 21, 22, and 24 have each found and cleaned up a stale or unmerged PR from an earlier run — if a PR is open at session start, verify and merge it before starting new work rather than re-deriving its scope). `node tests/smoke.js` catches data/logic regressions but nothing runs it in CI yet (Tier 3 gate still open; only `deploy.yml`'s post-deploy `verify` job exists, which just polls the live URL, and it only triggers on push to `main`, not on PRs). | ≤1 sonnet | pending |
...
**Then (order):** [x] Exodus 3(M), [x] 12(M), [x] 14(M), 19–20, 32 · Joshua 2, 6 · Judges 4, 7, 16 · Ruth 1–4 · 1 Kgs 3, 8, 17–19 · 2 Kgs 5, 18–19, 25 · Ezra 3 · Neh 2, 8 · Esther 4 · Job 1–2, 38 · Pss 22, 23, 51, 137 · Isa 6, 40, 53 · Jer 1, 29, 31 · Ezek 1, 37 · Dan 1–6 · Jonah 1–4 · Mic 5 · Hag 1 ·
...
**Datasets:** places 141/200 (unchanged in iteration 25 — no new places needed) · people 198/120 (target exceeded — target was a floor, not a cap; +1 from iteration 25: pharaoh-exodus) · objects 84/60 (target exceeded; +1 from iteration 25: hyssop)
