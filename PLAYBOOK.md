...
| 13 | 2026-07-12 | Re-scanned all GitHub issues (only #7 exists, still closed/already shipped — nothing new to slot; playbook backlog table already accurate, no changes needed). Curated 2 Sam 15 (Absalom's conspiracy, David's barefoot flight over the Mount of Olives, Ittai's oath, Hushai vs. Ahithophel, M) + 2 Sam 18 (forest of Ephraim battle, Absalom's death in the oak, Joab's disobedience, David's grief, M); new people `absalom`, `ahithophel`, `hushai`, `ittai`, `shimei`; new place `forest-ephraim`; new objects `mule`, `oak-tree`, `pillar-monument`; reused existing `jerusalem`/`hebron`/`olives`/`mahanaim`/`joab`; fixtures 10-15/10-18; sweep entries added (156 assertions, PASS) | 1× sonnet ENG (curation) | ✅ shipped |
| — | NEXT SESSION | Continue 2 Samuel key chapters (24 — M depth), then move to the Genesis/Exodus block; re-scan GitHub issues first | ≤1 sonnet | pending |
...
**2 Samuel:** [x] 1(M) · [x] 5(M) · [x] 6(M) · [x] 7(M) · [x] 11–12(M) · [x] 15(M) · [x] 18(M) · [ ] 24
...
**Datasets:** places 121/200 (+1 new: forest-ephraim; reused jerusalem/hebron/olives/mahanaim) · people 92/120 (+5 new: absalom, ahithophel, hushai, ittai, shimei) · objects 51/60 (+3 new: mule, oak-tree, pillar-monument)
...
| # | Issue | Scope | Slot |
|---|-------|-------|------|
| [#7](https://github.com/elinxie/Bible-verse-visualizer/issues/7) | Add translations to city names | ✅ **Shipped in iteration 7** (see Ledger); 3 more places (engedi/ziph/maon) topped up with `meaning` in iteration 8. Remaining ~65 non-curated places (jerusalem-adjacent minor sites, all Isa/Jer/Ezek/Rev-era towns, etc.) still lack `meaning` — a pure data top-up, safe to ride along with a future curation batch rather than needing its own iteration. | Repo re-scanned 2026-07-10: still the only issue ever filed, already closed — no open issues exist. |
