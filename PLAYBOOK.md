...
| — | NEXT SESSION | Re-scanned GitHub issues 2026-08-07: still 0 open (only #7 ever filed, closed, already shipped) — nothing new to slot. Joshua 2 and 6 are curated and on this iteration's branch, ready to merge. Continue the "Then" roadmap with **Judges 4, 7, 16** next (the next unchecked item after Joshua). Also worth a periodic check: confirm no further un-reviewed direct merges to main have introduced regressions, and watch for the duplicate-PR / unmerged-PR pileup pattern (iterations 21, 22, 24, and 26 have each found and cleaned up a stale or unmerged PR from an earlier run — if a PR is open at session start, verify and merge it before starting new work rather than re-deriving its scope). `node tests/smoke.js` catches data/logic regressions but nothing runs it in CI yet (Tier 3 gate still open; only `deploy.yml`'s post-deploy `verify` job exists, which just polls the live URL, and it only triggers on push to `main`, not on PRs). | ≤1 sonnet | pending |
...
**Then (order):** [x] Exodus 3(M), [x] 12(M), [x] 14(M), [x] 19(M), [x] 20(M), [x] 32(M) · [x] Joshua 2(M), 6(M) · Judges 4, 7, 16 · Ruth 1–4 · 1 Kgs 3, 8, 17–19 · 2 Kgs 5, 18–19, 25 · Ezra 3 · Neh 2, 8 · Esther 4 · Job 1–2, 38 · Pss 22, 23, 51, 137 · Isa 6, 40, 53 · Jer 1, 29, 31 · Ezek 1, 37 · Dan 1–6 · Jonah 1–4 · Mic 5 · Hag 1 ·
...
**Datasets:** places 141/200 (unchanged in iteration 27 — no new places needed) · people 200/120 (target exceeded — target was a floor, not a cap; +1 from iteration 27: rahab) · objects 88/60 (target exceeded; +1 from iteration 27: scarlet-cord)
