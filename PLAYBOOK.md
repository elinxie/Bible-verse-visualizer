...
| — | NEXT SESSION | Re-scanned GitHub issues 2026-08-12: still 0 open (only #7 ever filed, closed, already shipped) — nothing new to slot. 1 Kings 3, 8 is on `main` and live (pending this iteration's deploy). Continue the "Then" roadmap with **1 Kings 17–19** next (Elijah at the brook Cherith / the widow of Zarephath, the Mount Carmel contest with the prophets of Baal, and Elijah's flight to Horeb — the next unchecked items after 1 Kings 3, 8). Also worth a periodic check: confirm no further un-reviewed direct merges to main have introduced regressions, and watch for the duplicate-PR / unmerged-PR / draft-PR-left-unmarked-ready pileup pattern (iterations 21, 22, 24, 26, 28, 29, 30, and 32 have each found and cleaned up a stale, unmerged, or still-draft PR from an earlier run — if a PR is open at session start, verify it locally, mark it ready if it's a draft, and merge it before starting new work rather than re-deriving its scope). `node tests/smoke.js` catches data/logic regressions but nothing runs it in CI yet (Tier 3 gate still open; only `deploy.yml`'s post-deploy `verify` job exists, which just polls the live URL, and it only triggers on push to `main`, not on PRs). Note: fresh containers may not have `playwright` pre-installed under the repo — `npm i playwright --no-save` (a few seconds, uses the preinstalled Chromium at `/opt/pw-browsers`) is needed before `node tests/smoke.js` will run. | ≤1 sonnet | pending |
...
**Then (order):** [x] Exodus 3(M), [x] 12(M), [x] 14(M), [x] 19(M), [x] 20(M), [x] 32(M) · [x] Joshua 2(M), 6(M) · [x] Judges 4(M), 7(M), 16(M) · [x] Ruth 1–4(M) — FULL BOOK COMPLETE · [x] 1 Kgs 3(M), 8(M) · 1 Kgs 17–19 · 2 Kgs 5, 18–19, 25 · Ezra 3 · Neh 2, 8 · Esther 4 · Job 1–2, 38 · Pss 22, 23, 51, 137 · Isa 6, 40, 53 · Jer 1, 29, 31 · Ezek 1, 37 · Dan 1–6 · Jonah 1–4 · Mic 5 · Hag 1 ·
...
**Datasets:** places 144/200 (unchanged in iteration 31 — 1 Kings 3/8 fully reused `gibeon`/`jerusalem`/`egypt`) · people 216/120 (target exceeded — target was a floor, not a cap; +2 from iteration 31: pharaohs-daughter, two-mothers) · objects 98/60 (target exceeded; +1 from iteration 31: temple-glory-cloud)
...
| # | Issue | Scope | Slot |
|---|-------|-------|------|
| [#7](https://github.com/elinxie/Bible-verse-visualizer/issues/7) | Add translations to city names | ✅ **Shipped in iteration 7** (see Ledger); 3 more places (engedi/ziph/maon) topped up with `meaning` in iteration 8; `eden` shipped with `meaning` from birth in iteration 15; all iteration 22 places (gob/zela/abel-beth-maacah) shipped with `meaning` from birth too. Remaining ~65 non-curated places (jerusalem-adjacent minor sites, all Isa/Jer/Ezek/Rev-era towns, etc.) still lack `meaning` — a pure data top-up, safe to ride along with a future curation batch rather than needing its own iteration. | Repo re-scanned 2026-07-31: still the only issue ever filed (open+closed), already closed — no open issues exist. |
